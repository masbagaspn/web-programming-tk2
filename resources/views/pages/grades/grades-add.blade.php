...
@section('content')
    <div class="w-full grow flex flex-col">
        ...
        <form action="{{ route('grades.create', $student->id) }}" method="POST" class="w-full grow flex flex-col gap-20 items-between justify-between">
           {{ csrf_field() }}
           <div class="w-full grid grid-cols-3 gap-8 p-8">
                @foreach($grades as $grade)
                <div class="input-wrapper">
                    <label class="input-label">{{ ucfirst($grade->grade_name) }} Score</label>
                    <input name="{{ $grade->id }}" type="number" min="0" max="100" class="appearance-none input-field"/>
                </div>
                @endforeach
           </div>
